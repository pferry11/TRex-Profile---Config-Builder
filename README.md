...
that can be re-imported and edited later. Generated file headers include the
exact `stl-sim` / `astf-sim` command to validate on a TRex box **and a
plain-English `# Summary:` of what the profile does** — the same summary shown
live in the "What this does" box above every output. Hover the ⓘ icons for
field-by-field help.
