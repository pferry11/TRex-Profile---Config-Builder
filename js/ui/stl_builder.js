...
(function (root) {
  ...
  TB.ui.stlBuilder = {
    mount: function (container) {
      ...
      function regen() {
        ...
        regenTimer = setTimeout(function () {
          ...
          TB.ui.output.render(outputPane, { result: gen(model), model: model, validateKind: 'stl',
            /* Re-open lives in the output pane (same pattern as cap2). Two
               distinct operations: "Open profile…" parses a generated (or
               hand-written) .py from its body; "Open builder file…" restores a
               .json builder file exactly. */
            profileAccept: '.py',
      ...
      /* Re-import a generated STL .py profile. Values are read from the file body
         (so hand-edits are honoured); the profile name comes from the file name.
      ...
      function reportImport(res, filename) {
        ...
        if (res.coverage === 1 && !res.unmapped.length) {
          TB.ui.toast('Imported "' + filename + '"' + (res.source === 'tool'
            ? ' - tool-generated, all ' + res.total + ' constructs mapped.'
            : ' - mapped all ' + res.total + ' recognised constructs.'), 'ok');
          return;
        }
        var lead = res.source === 'tool'
          ? 'This profile carries lines the app doesn\'t recognise (a hand-added or newer construct). Everything it knows was imported; the lines below were left out:'
          : 'This file wasn\'t generated by the builder, so it was parsed best-effort. STL profiles are arbitrary Python, so anything beyond the simple shapes the tool emits may not map. Everything recognised was imported; the lines below had no matching field:';
        var banner = TB.ui.el('div', { class: 'warn-banner' }, [
          TB.ui.el('div', {}, [TB.ui.el('strong', { text: 'Partial import - ' + pct + '% mapped (' + res.mapped + ' of ' + res.total + ' lines).' })]),
          TB.ui.el('div', { text: lead }),
          TB.ui.el('ul', {}, res.unmapped.slice(0, 40).map(function (l) { return TB.ui.el('li', { text: l }); })
            .concat(res.unmapped.length > 40 ? [TB.ui.el('li', { text: '…and ' + (res.unmapped.length - 40) + ' more.' })] : []))
        ]);
        var body = TB.ui.el('div', {}, [banner,
          TB.ui.el('p', { text: 'Everything that mapped has been loaded into the builder - review the fields, then regenerate.' })]);
