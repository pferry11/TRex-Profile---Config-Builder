    ...
    mount: function (container) {
      ...
      function regen() {
        ...
        regenTimer = setTimeout(function () {
          ...
          TB.ui.output.render(outputPane, { result: gen(model), model: model, validateKind: 'astf',
            /* Re-open lives in the output pane (same pattern as STL/cap2). Two
      ...
      function renderTopbar() {
        ...
        /* Re-open moved to the output pane's "Open builder file…" / "Open
           profile…" actions (same split as STL/cap2) - no topbar Import button. */
      ...
      /* Re-import a generated ASTF .py profile. Values are read from the file
         body (hand-edits honoured); the name comes from the file name. A file
      ...
      function reportImport(res, filename) {
        ...
        var lead = res.source === 'tool'
          ...
          : 'This file wasn\'t generated by the builder, so it was parsed best-effort. ASTF profiles are arbitrary Python, so anything beyond the simple shapes the tool emits may not map. Everything recognised was imported; the lines below had no matching field:';
        var banner = el('div', { class: 'warn-banner' }, [
          el('div', {}, [el('strong', { text: 'Partial import - ' + pct + '% mapped (' + res.mapped + ' of ' + res.total + ' lines).' })]),
          el('div', { text: lead }),
