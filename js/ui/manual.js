  ...
  var CHAPTERS = [
    { id: 'start', title: 'Getting started', html: function () {
      return '<p>This app builds Cisco TRex configuration artifacts through forms: traffic profiles ' +
        ...
        'guaranteed to map fully. The <strong>STL</strong> tab now offers the same <strong>Open profile…</strong> for ' +
        'generated <code>.py</code> profiles (they round-trip exactly); hand-written STL is arbitrary Python so the ' +
        'offline parser maps it best-effort. <em>With the Flask backend running</em>, Open profile… instead ' +
        '<strong>executes</strong> the profile server-side and hands back the concrete streams - so comprehensions, ' +
        '<code>__init__</code> tables and conditionals load faithfully, with anything unmodelled kept as a raw-scapy ' +
        'expression. ASTF <code>.py</code> re-import is still on the roadmap - see <em>Future updates</em>.</p>' +
    ...
    { id: 'future', title: 'Future updates', html: function () {
      ...
      var rows = [
        ...
        ['App platform', 'Re-import .py profiles (STL/ASTF)', 'Extend the same body-is-truth re-edit seam to the Python builders. STL is done (v0.26.3): a generated .py re-imports via the output-pane Open profile… action and round-trips byte-identically - the # trexb: stl tag marks tool files and per-field readers reverse every packet layer, field-engine op, TX mode, flow-stat and tunable. Hand-written STL is arbitrary Python, so the offline parser maps it best-effort: as of v0.26.4 it fully loads 22 of the 106 shipped stl/*.py files and extracts streams from most of the rest (following base_pkt/pkt/vm assignments, fluent and low-level field engines, multi-stream STLProfile lists), preserving anything non-static as raw scapy. As of v0.26.5 the backend Python resolver is shipped too: with the Flask backend up, Open profile… executes the profile server-side (tools/stl_resolve.py runs it against a recording stand-in for the TRex API plus real scapy) and resolves 72 of the 106 shipped files - arbitrary comprehensions/__init__ tables/conditionals included - falling back to the offline parser when the backend is absent. ASTF is following the same path: as of v0.27.3 a generated ASTF .py re-imports via Open profile… and round-trips byte-identically (ip generator, per-side TCP tuning, payload pool, program command lists, template wiring); the offline best-effort pass fully maps 65 of the 98 shipped astf/*.py files (v0.27.5). And the ASTF execute-not-parse resolver shipped in v0.27.6 (tools/astf_resolve.py, POST /api/import_profile kind=astf): with the Flask backend up, Open profile… executes the profile against recording stand-ins for the ASTF API + real scapy and resolves 85 of 98 shipped files - tunable/conditional ones included - falling back to the offline parser otherwise. Re-import is now complete for every format: cap2 .yaml, STL .py and ASTF .py.', 'Large', 'done'],
        ...
        ['Test process', 'Tiered Robot Framework harness', 'A test plan spanning unit -> static -> fidelity -> simulation -> UI -> real hardware, orchestrated by Robot Framework and run on demand from the dev PC against the lab over SSH (tests/robot/, run with tests\\robot\\run.ps1). Tiers 0-2 are shipped and need no lab: T0 wraps the existing tests.html goldens in a real browser (the page now publishes window.__TB_TESTS so the result is machine-readable); T1 generates every committed fixture through the real generators in Node and proves each artifact is valid input for whatever consumes it - Python compiles, YAML/JSON parse, BIRD braces balance, trex_cfg port_limit agrees with the interface count; T2 turns the documented import-coverage baselines into gates (the three *_import_coverage.js tools gained --json and --min-full, sharing tools/coverage_cli.js) and asserts every tool-generated profile re-imports and regenerates byte-identically, with documented gaps like the GTP-U companion _topo.py tracked as known limitations that fail if they ever start working. Fixtures are derived, not hand-written: captured from tests.html (where goldens already pin them) by a re-runnable maintenance task, so they cannot drift from the app.', 'Large', 'in-progress'],
        ['Test process', 'Lab tiers (offline sim, UI, real traffic)', 'T3 runs stl-sim/astf-sim on the box over SSH and through the builder\'s own /api/validate; T4 drives all 11 tabs headlessly via the Browser library (build, save/reload, re-import round trip, undo/redo, persistence, workspace export/import); T5 is the spine - build a profile, push it to the box, run it via TRex\'s own STLClient/ASTFClient and assert on get_stats(). The traffic driver is an on-box agent invoked over SSH rather than a Robot library, because the TRex tree\'s bundled scapy/pyzmq are Linux-only and will not import on a Windows dev PC. One test body serves all three lab topologies (looped box, box + DUT, two boxes) - only the thresholds in variables/env_*.py change. Pass criteria are functional plus a loose rate tolerance, deliberately not a benchmark.', 'Large', 'planned'],
        ['Test process', 'Manual usability walkthroughs', 'Eleven per-tab walkthroughs (tests/manual/) covering every tab including TPG and BIRD, whose builder UI is testable even though their lab tier is not. Each has a cold-start task stated in the tester\'s language rather than the app\'s, timed observation of hesitations and wrong turns, ten common checkpoints plus per-tab additions, and an edit-an-existing-artifact task that probes the profile vs builder-file distinction. Output is a severity-rated findings log, not a pass/fail; S1/S2 findings graduate into this very table, so usability work is planned alongside feature work with no second backlog. The value depends on the tester not being the person who built the tab - a pass yielding zero findings across 11 tabs means the tasks were too leading, not that the UI is perfect.', 'Medium', 'in-progress'],
        ['Test process', 'Traceability dashboard', 'tools/test_matrix.js reads a coverage map plus Robot\'s output.xml and emits a dependency-free tests/matrix.html in the same style as tests.html: one row per builder feature, columns per tier, and a usability column showing each tab\'s last walkthrough date and open S1/S2 count. Makes the TPG/BIRD lab gaps and stale usability passes visible rather than silent.', 'Small', 'planned']
      ];
