  ...
  TB.help = {
    ...
    cap2: {
      ...
      _sections: {
        ...
        pools: 'Optional named client/server sub-ranges (generator_clients/generator_servers). A pcap template pins its IPs to a pool via client_pool/server_pool instead of the global range - lets different templates use different subnets in one profile.',
      ...
      importFile: 'Re-open a file in the output pane: "Open profile…" reads a TRex profile (.yaml) - values are read from the file body, so edits you made (a changed IP or rate) come straight back in; files this tool generated map fully, hand-written/third-party .yaml import best-effort with a coverage report. "Open builder file…" restores this app\'s own .json save (an exact reload).',
      ...
      mac: 'Base source MAC for generated packets, as six hex bytes (00:00:00:01:00:00). Emitted as the global mac : [..] list. Leave blank to use the port config.',
      poolName: 'Pool label (e.g. c1, s1) that a pcap template references via client_pool/server_pool. Unique per side.',
      trackPorts: 'Server-pool flag: keep per-flow source-port state so responses map back to the right flow. Emitted as track_ports (server pools only).',
      poolRef: 'Pin this template\'s client/server IPs to a named generator pool instead of the global range. Leave on (global range) to use the top-level clients/servers ranges. Pools are defined in the Per-template generator pools section above.',
      oneAppServer: 'Send every flow from this template to a single server address (server_addr) instead of spreading across the server range - matches the shipped dns_one_server / lb_ex1 examples.',
      serverAddr: 'The single destination IP used when one_app_server is on.',
      ipv6: 'Base IPv6 address for src/dst when replaying over IPv6, as six hex words (the generator fills the rest). Emitted as src_ipv6 / dst_ipv6 : [0x..,..]. See the shipped ipv6.yaml.',
      minMaxIp: 'Override the client/server IP range with explicit 32-bit bounds (min_src_ip/max_src_ip/min_dst_ip/max_dst_ip). Enter a dotted IPv4 (16.0.0.1) or raw hex (0x10000001) - either way it is stored and emitted as the hex TRex expects. Used by rtsp_full1 / sfr_agg.',
      tw: 'Timer-wheel tuning for very large flow counts: buckets, levels and bucket_time_usec (matches the shipped dns_tw.yaml). Leave off unless you know you need it.',
